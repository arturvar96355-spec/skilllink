...
export interface UserDto {
  ...
  isActive: boolean
  /**
   * Учётная запись эксперта хакатона (решение 147). Эксперт формально может носить
   * роль ADMIN, MANAGER или HEAD (RESPONSIBLE_ROLES), но ответственным его назначать
   * нельзя: сервер (`assertStaffResponsible`) такое назначение всё равно отклонит.
   * Показывается только тем, кто видит employees-справочник, чтобы отличить эксперта
   * в списке пользователей от обычного сотрудника.
   */
  isReviewer: boolean
  /**
   * Можно ли назначить ответственным за вуз, связку, этап, встречу или документ —
   * `canBeResponsible(role)` и одновременно не эксперт (`!isReviewer`). Считается на
   * сервере, чтобы фронт не держал вторую копию этого правила рядом с ролью.
   */
