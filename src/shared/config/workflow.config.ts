...
export interface StageDefinition {
   ...
   * Нормативы по всем этапам — решение команды (PM) от 26.09.2026, формально заказчиком не утверждено.
   */
  normativeDays: number
  /** Этап выполняется при необходимости: его отменяют с комментарием «не требуется». */
  optional: boolean
 ...
 *
 * Порядок этапов в целом остаётся свободным: в жизни они идут
 ...
 *
 * Состав контрольных точек (6, 7, 11) — решение команды (PM) (docs/DECISIONS.md, решение 13),
 * разбор — docs/CONTROL_POINTS.md.
 */
export const CONTROL_POINT_STAGES: readonly number[] = [6, 7, 11]

/**
 * 14 этапов из раздела 8 ТЗ. При создании связки создаются все сразу.
 * Чек-листы — TEMP: состав пунктов — рабочее значение, согласуется с заказчиком.
 * Состав сверен с заказчиком (docs/DECISIONS.md, решение 12), разбор — docs/CHECKLISTS.md.
 */
export const WORKFLOW_STAGES: readonly StageDefinition[] = [
  ...
  },
  {
    ...
    normativeDays: 240,
    optional: false,
    // Пунктов нет: этап вычисляется автоматически по состоянию этапов 1–13.
    tasks: [],
  },
]
