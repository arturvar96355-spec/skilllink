 ...
 *   пятью неверными паролями закрывал вход владельцу, где бы тот ни был, — демо-вход
 ...
 * Раньше новая запись в полную карту просто не попадала: после десяти тысяч
 * случайных адресов перебор по любой новой учётной записи не считался вовсе.
 */
class ThrottleStore {
 ...
 * идут подряд, без `await` между ними, а удачный вход счётчик учётной записи
 * обнуляет — поэтому верный пароль с пятой попытки по-прежнему пускает.
 *
