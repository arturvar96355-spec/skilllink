...
describe('хранилище попыток', () => {
  ...
  it('чужой адрес не закрывает вход владельцу учётной записи', () => {
    // Счёт по одной учётной записи позволил бы пятью неверными паролями с любого
    // ноутбука закрыть демо-вход менеджера всем, включая его самого.
  ...
  it('полная карта вытесняет самые старые записи, а не перестаёт считать новые', () => {
    ...
    // Новая учётная запись обязана попасть в полную карту, иначе её перебор
    // не ограничивается вовсе.
...
describe('одновременные попытки входа', () => {
  ...
  it('сто одновременных догадок — проверяется не больше пяти', async () => {
    // Если записывать неудачу после запроса к базе и bcrypt, все сто пройдут
    // проверку блокировки раньше, чем первая успеет записаться.
    let checked = 0
