...
export async function findOverdue(
  ...
  // (решение 47). По 24 часа этап с бейджем «−1 дн.» не попал бы в выборку
  // `minDaysOverdue=1`, пока не пройдут сутки с часа срока.
 ...
 * и запрещает. Иначе закрытая связка оставалась бы в списках просроченных навсегда.
 */
function buildCooperationFilter(
  ...
  //
  // С `scope.universityId ?? query.universityId` представитель вуза, спросивший
  // о чужом вузе, получил бы СВОИ записи под чужой подписью. Утечки нет, но ответ
  // отвечает не на заданный вопрос — а это хуже пустого ответа, потому что
  ...
  return {
    // Контрольный этап вычисляется автоматически и вручную не меняется.
 ...
 * точка — по предыдущим, завершение — по чек-листу. Транзакция сама по себе
 * ничего не блокирует: два менеджера, одновременно закрывшие этапы 12 и 13,
 * оставили бы этап 14 «в работе» при закрытых 1–13 — каждый видит чужой этап ещё
 * открытым. Снятие обязательного пункта одновременно с завершением этапа дало бы
 * завершённый этап с незакрытым пунктом.
 *
...
/**
 * Пересчёт контрольного этапа 14 по состоянию этапов 1–13.
 ...
 *
 * Обещана в SECURITY_LIMITATIONS. Пишется после транзакции:
