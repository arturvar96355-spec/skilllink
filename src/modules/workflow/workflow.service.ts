...
export interface StageDtoOptions {
  /** Скрыть внутренние комментарии сотрудников: представитель вуза их не видит. */
...
export function toStageDto(
  ...
  return {
    ...
    tasks: row.tasks.map((task) => ({
      ...
      // «письмо от И. И. Иванова», а вуз внутренних заметок не видит.
...
/**
 * Проверяет, что связка видна пользователю (представителю вуза — только своего вуза),
 * и возвращает её статус.
...
export async function updateStage(
  ...
  // Одного внешнего ключа мало: он пропустит аналитика, наблюдателя и представителя
  // вуза, а на несуществующий id ответит ошибкой базы.
  ...
  if (statusChanged) {
    ...
    // Правка полей без смены статуса тоже должна оставлять след: перенос срока
    // у уже завершённого этапа переписывает показатель «этапы, закрытые в срок»,
    // и без записи в журнале не узнать, почему показатель изменился.
    await writeAudit({
      userId: user.id,
 ...
 * представителя вуза, подтверждающего получение материалов. Своя запись у кабинета
 * вуза обходила бы эти правила: отмечала бы пункты завершённого этапа и не вставала
 * в очередь со сменой статусов. Проверки прав и видимости остаются у вызывающего;
 * здесь — то, что должно быть одинаковым у всех.
 *
...
export async function setTaskDone(
  ...
  return prisma.$transaction(async (tx) => {
    ...
    // снимут в тот же миг, когда этап завершают, и завершённый этап останется
    // с незакрытым обязательным пунктом.
    await repo.lockCooperation(tx, target.cooperationId)
    // Повторная отметка уже отмеченного пункта ничего не меняет и не переписывает,
    // кто и когда его отметил: иначе подтверждение получения материалов представителем
    // вуза перешло бы к менеджеру, нажавшему на устаревшей странице.
 ...
 * сами (решение 87): подпись не вводится дважды — статусом документа и галочками
 * в чек-листе.
 *
...
export async function toggleTask(
  ...
  if (changed) {
    ...
    // Отметка в чек-листе — движение по связке: «без движения» уже неправда.
