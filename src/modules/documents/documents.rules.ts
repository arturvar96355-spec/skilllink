 ...
 * решает, какие действия предлагать. Реэкспорт — для правил и тестов модуля.
 */
export { ALLOWED_DOCUMENT_TRANSITIONS }
 ...
 *
 * Правило проверяет итог любой записи — и перехода, и правки. Если проверять только
 * отправку на согласование, у утверждённого документа без текста можно стереть ссылку
 * и затем подписать — в системе появится подписанный документ, которого нет.
 */
 ...
 *
 * Сверка только по ключу шаблона ловит лишь собранное из шаблона. Договор, заведённый
 * вручную, или новая версия собранного (у неё ключа нет) прошли бы мимо — и пакет
 * добавил бы к связке второй договор. Поэтому совпадением считается и документ того же
 ...
 * договор и лицензия. Пункты этапа отмечает подписанный договор;
