...
**Отказ во входе — два кода**, приходят в `code` ответа `signIn` (или в адресе
...
(`setTaskDone`), в очереди со сменой статусов связки. Повторное подтверждение уже
подтверждённого — 200 и в завершённом этапе: ничего не меняет и в журнал не пишется. В ответе — обновлённый список материалов.
    ...
    "groups": [
      ...
      { "type": "cooperation", "title": "Сотрудничества", "total": 1,
        "items": [ { "type": "cooperation", "id": "…",
                     "title": "Московский технический университет связи и информатики — Облачные технологии и инфраструктура",
                     "subtitle": "Облачная платформа РТК · В работе" } ] }
    ]
  }
}
```
