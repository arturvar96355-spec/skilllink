...
`commit` — короткий хеш коммита выкладки, ставит сборка через `APP_COMMIT`
(Dockerfile/deploy.sh/remote-up.sh); `null`, если переменной нет (например, локальный
`npm run dev`) — риск 13 ревизии от 26.09.2026: без этого нельзя было проверить, что
сейчас на стенде. `version` — версия из `package.json`. Оба поля не прячутся в
промышленном режиме — это не секрет, а как раз то, что должно быть видно снаружи.
...
| `GET /api/auth/providers` | список провайдеров: вход по паролю всегда, `expert` — только пока включена `EXPERT_QUICK_LOGIN` |
| `POST /api/auth/callback/credentials` | вход: форма с `csrfToken`, `email`, `password` |
| `POST /api/auth/callback/expert` | быстрый вход экспертов (решение 176): форма с `csrfToken`, `account` (`manager` \| `admin` \| `rep`) — без пароля. Выключено `EXPERT_QUICK_LOGIN` — 404, как «нет такого адреса», а не отказ провайдера |
...
сообщений можно было бы перебирать существующие адреса. Быстрый вход экспертов ведёт
себя так же: неизвестный ключ `account`, учётная запись без `users.is_reviewer = true`
и заблокированная попытка (`too_many_attempts`, тот же счётчик перебора, что у обычного
входа) — все три отвечают одинаковым отказом `CredentialsSignin`, кроме заблокированной
попытки, у которой, как и при обычном входе, свой `code: too_many_attempts`.
...
**Прочитанность хранит сервер** (решение 139): время последнего просмотра — в
`users.notifications_seen_at`, ставится вызовом `POST /api/notifications/seen`.
Без `since` от клиента `GET /api/notifications` берёт это серверное значение сам.
`since` в запросе остался для обратной совместимости со старым фронтом (быстрый
локальный кэш в `localStorage`) — если он передан, используется более позднее
из двух значений, так что подсказка клиента не может сделать ленту «более
прочитанной», чем на сервере. Всё новее итоговой границы приходит с `isUnread: true`.
Без какой-либо отметки (ни на сервере, ни в `since`) непрочитанным считается всё.
...
Право: `READ`. Отмечает ленту уведомлений просмотренной — ставит
`users.notifications_seen_at` (решение 139). Тела нет — сервер ставит своё текущее
время; с телом `{ "seenAt": "…ISO 8601…" }` — переданное время, если оно не позже
текущего времени сервера (иначе 422). Ответ — `{ "data": { "seenAt": "…" } }`.

Раньше отметка «прочитано» жила только в `localStorage` браузера: на другом
устройстве или после очистки хранилища вся лента снова становилась «новой» — это и
была жалоба, из-за которой появился этот маршрут. Теперь `localStorage` на фронте —
только быстрый локальный кэш, а источник истины — этот маршрут и серверное значение,
которое читает `GET /api/notifications`.
...
```json
{
  "data": {
    ...
    "requestId": "…",
    "operator": { "name": "ИТ-Школа РТК (реквизиты указываются при внедрении)", "address": "ИТ-Школа РТК (реквизиты указываются при внедрении)", "responsibleContact": "ИТ-Школа РТК (реквизиты указываются при внедрении)", "note": "…" },
...
- `operator` — реквизиты оператора ПД (решение 116/139): `OPERATOR_NAME`, `OPERATOR_ADDRESS`,
  `OPERATOR_CONTACT` (.env.example). Без них — понятная заглушка «ИТ-Школа РТК (реквизиты
  указываются при внедрении)»: система работает и без реальных реквизитов, но настоящий
  ответ субъекту требует их вписать при внедрении.
...
## 15л. Отчёты «по ТЗ» и «Каталог по ТЗ» (решения 145, 172)

Право — `READ` у обоих (как у самого реестра связок), представителю вуза — только свой вуз.
Оба принимают `?format=csv|xlsx|json`, без формата — `csv`, и одинаковый набор фильтров
(ниже) — фиксирован набор колонок дословно из ТЗ, фильтры отбирают строки, а не колонки.
Не путать с существующей выгрузкой `GET /api/export?dataset=cooperations` (раздел 15г):
та выгружает реестр связок целиком со своими фильтрами и колонками.

### Фильтры (решение 172, ТЗ заказчика — «отчёты формируются с фильтрами по периоду, вузу, ИТ-направлению, ИТ-продукту и ответственному»)

Все параметры необязательны, задаются вместе с `format`:

| Параметр | Тип | Смысл |
| --- | --- | --- |
| `dateFrom` | дата `2026-01-01` или ISO 8601 | Начало периода |
| `dateTo` | дата `2026-06-30` или ISO 8601 | Конец периода |
| `universityId` | id | Вуз |
| `programId` | id | ИТ-направление (образовательная программа) |
| `productId` | id | ИТ-продукт |
| `responsibleId` | id | Ответственный |
| `status` | `DRAFT\|ACTIVE\|PAUSED\|COMPLETED\|CANCELLED` | Статус работы с вузом |

Дата без времени разворачивается в начало/конец московских суток UTC. `dateFrom` позже
`dateTo` — `VALIDATION_ERROR` 422 с полем `dateFrom`. Период — по **активности связки**,
не по дате её создания: связка попадает в отчёт, если к концу периода она уже
существовала (`createdAt <= dateTo`) и не была закрыта до его начала (`closedAt` пуст или
`>= dateFrom`) — то есть была действующей хотя бы день внутри периода. Запрос вуза вне
области видимости представителя даёт пустой отчёт, а не ошибку доступа.

Пример: `GET /api/reports/tz?format=xlsx&dateFrom=2026-01-01&dateTo=2026-06-30&status=ACTIVE`.

Фильтры отражаются и в самом файле: json несёт их в поле `filters` (см. ниже), csv и xlsx —
строкой-заголовком перед колонками («Фильтры: период 01.01.2026 — 30.06.2026 · статус
«В работе»» — вуз/программа/продукт/ответственный резолвятся в имя, если фильтр доступен),
а имя файла получает необязательный хвост с периодом и статусом (например,
`skilllink-tz-report-2026-09-26_from-2026-01-01_to-2026-06-30_status-active.xlsx`) —
вуз/программу/продукт/ответственного в имя файла не кладём, их не прочитать без справочника.
...
```json
{
  ...
  "filters": { "status": "ACTIVE", "dateFrom": "2025-12-31T21:00:00.000Z", "dateTo": "2026-06-30T20:59:59.999Z" },
...
`filters` — ровно те параметры запроса из раздела выше, что были заданы (даты — уже
развёрнутые в полный ISO 8601); без единого фильтра — пустой объект, как и раньше.
Существующий печатный `pdf` — не отдельный формат этих эндпоинтов: это печать листа
браузером («Печать / PDF»), как у «Отчёта руководителю» (`/reports/portfolio`); печать
показывает применённые фильтры в шапке листа.

Все три ответа — `Cache-Control: no-store` (в отчётах — рабочие данные и ФИО).

---

## 15м. Письма вузов (решение 170)

«Письма вузов» ТЗ, вариант 2: письмо становится **обращением**. Живого почтового ящика
нет — ни IMAP, ни SMTP: письмо приходит демо-набором или загрузкой файла `.eml`. Систему
разбирает код (вуз по домену отправителя, связка, этап) и, если подключена и настроена
модель (YandexGPT/GigaChat), она — группу и предлагаемое действие; без модели или при её
сбое — запасной путь по ключевым словам. ADMIN или HEAD подтверждают разбор («Верно») или
исправляют («Неверно») — это решение создаёт **задание** ответственному за вуз (отдельная
простая таблица `inbound_letter_tasks`, не запись в ленте рекомендаций — почему, см.
`docs/TECHNICAL_DECISIONS.md`, решение 170) и одновременно размеченный пример, на котором
учится разбор следующих писем — точность по группе (`GET …/stats`) и подсказка модели
похожими письмами (few-shot). Ответ вузу — только черновик (`replyDraft`), отправки нет:
кнопка «Открыть в почте» использует ссылку `mailto:`.

**Права:** `INBOUND_READ` (ADMIN, HEAD, MANAGER) — список и карточка; MANAGER видит только
обращения вузов, где он ответственный (за сам вуз или за найденную связку), остальным
...
«Прочее». **Статусы:** `NEW` → `ANALYZED` → `CONFIRMED` («Верно») | `CORRECTED` («Неверно»)
| `DISMISSED` (спам); из трёх последних переходов нет.

**DTO письма** (`GET /api/inbound-letters/:id` и элемент списка) несёт разбор дважды:
`detected` — что нашёл код и модель/правила, не переписывается проверкой; `current` —
действующая версия (до «Неверно» совпадает с `detected`, после — то, что указал сотрудник).
У каждого — `universityId`/`universityName`/`cooperationId`/`stageNumber`/`group`/`action`,
у `detected` ещё `confidence` (0..1), `quotes` (цитаты-основания), `analyzedBy`
(`MODEL`/`RULES`) и `fallbackReason` (как у ИИ-помощника, решение 90 — `disabled`,
`not-configured`, `invalid` и так далее, только у разбора правилами). `review` — кто,
когда, «Верно»/«Неверно», комментарий; `task` — короткая сводка задания (ответственный,
статус, заголовок); `replyDraft` — текст, источник (`yandexgpt`/`gigachat`/`template`),
время правки и `mailto`. Элемент списка (`GET /api/inbound-letters`) — то же самое без
`bodyText` (вместо него `bodyPreview`, ~160 символов) и без `replyDraft`.
...
Право: `INBOUND_READ`. Полная карточка — см. DTO выше.

### POST /api/inbound-letters/upload

Право: `INBOUND_REVIEW`. Тело — `multipart/form-data`, поле `file` — файл `.eml`.
Предел размера — `MAX_EML_SIZE_BYTES` (5 МБ). Разбор заголовков — свой, без новых
...
### POST /api/inbound-letters/:id/analyze

Право: `INBOUND_REVIEW`. Разобрать заново. Доступно, пока обращение не проверено
(`NEW` или `ANALYZED`), иначе `CONFLICT` 409. Код определяет вуз по домену адреса
отправителя (сайт вуза или почта его контактных лиц — личная почта на общем провайдере,
`gmail.com` и подобные, никого не выдаёт), связку вуза, с которой сейчас работают
(открытая, предпочтительно активная), и её текущий этап. Группу и действие — модель,
если подключена, настроена и её ответ прошёл проверку JSON-схемой (промпту передаются
до трёх похожих прошлых размеченных писем — few-shot, поиск похожих словами текста,
TF-IDF и косинусное сходство, без внешних зависимостей); иначе — правила по ключевым
словам (`analyzedBy: "RULES"`, `fallbackReason` — почему не модель). Персональные данные
маскируются перед отправкой в модель (`ai-assist.privacy.ts`, решение 90) — тот же
редактор, что у ИИ-помощника.

```bash
curl -s -X POST http://localhost:3000/api/inbound-letters/<id>/analyze
```

```json
{
  "data": {
    "id": "…", "senderEmail": "priemnaya@university.example.invalid", "senderName": null,
    "subject": "Об этапе договора", "bodyText": "…", "receivedAt": "…",
    "source": "EML_UPLOAD", "messageId": "…", "status": "ANALYZED",
    "current": {
      "universityId": "…", "universityName": "СПбГУТ", "cooperationId": "…", "stageNumber": 6,
      "group": "DOCUMENTS", "action": "Проверить и оформить документы, о которых пишет вуз",
      "confidence": 0.55, "quotes": ["Просим выслать оригинал договора для подписания."],
      "analyzedBy": "RULES", "fallbackReason": "not-configured", "analyzedAt": "…"
    },
    "detected": { "...": "то же самое — совпадает с current до проверки" },
    "review": null, "task": null, "replyDraft": null,
    "isMock": false, "createdAt": "…", "updatedAt": "…"
  }
}
```

### POST /api/inbound-letters/:id/review

Право: `INBOUND_REVIEW`. Доступно, пока не проверено, иначе `CONFLICT` 409.

«Верно» — разбор принят как есть:

```json
{ "verdict": "CORRECT" }
```

Требует, чтобы разбор уже нашёл вуз (`detected.universityId` не пусто) — иначе
`VALIDATION_ERROR` 422 с подсказкой использовать «Неверно» и указать вуз вручную.

«Неверно» — сотрудник указывает правильные значения, комментарий обязателен:
...
```json
{
  ...
  "comment": "Вуз определён неверно — почта личная, не рабочая; на самом деле это МТУСИ"
}
```

`cooperationId` необязателен; если указан — проверяется, что связка принадлежит
указанному вузу (`VALIDATION_ERROR` 422 иначе). Оба варианта: статус → `CONFIRMED`
(«Верно») или `CORRECTED` («Неверно»); создаётся задание ответственному за вуз или
связку (см. ниже) и пишется событие в статистику точности по группе (`GET …/stats`,
сравнивается группа, которую нашёл разбор, с той, что подтвердил сотрудник, — вне
зависимости от того, что было не так с вузом).
...
```json
{ "comment": "Рассылка, не по вузу" }
```

Статус → `DISMISSED`. Задание не создаётся, событие в статистику точности не пишется —
это не пример на разбор, а «письмо не по работе».

### POST /api/inbound-letters/:id/reply-draft

Право: `INBOUND_REVIEW`. Собирает черновик ответа моделью (если подключена) или шаблоном
по действующей группе и действию (`current`, а до проверки — `detected`). Недоступно для
`NEW` (сначала разберите) и `DISMISSED` (отвечать не на что) — `CONFLICT` 409.

```bash
curl -s -X POST http://localhost:3000/api/inbound-letters/<id>/reply-draft
```
...
```json
{
  "data": {
    "replyDraft": {
      ...
      "source": "template",
      "updatedAt": "…",
      "mailto": "mailto:priemnaya%40university.example.invalid?subject=Re%3A%20…&body=…"
    }
  }
}
```
...
Право: `INBOUND_REVIEW`. Правка текста вручную:

```json
{ "text": "Уважаемые коллеги!\n\n…\n\nС уважением,\nИТ-Школа РТК" }
```

Источник у отредактированного вручную черновика — `template` (не приписывается модели,
даже если правка началась с её текста).

### GET /api/inbound-letters/stats

Право: `INBOUND_READ`. Точность разбора по каждой из шести групп — тот же принцип
забывания, что у обучения рекомендаций (решение 119, полупериод 30 дней,
`recommendations.learning.ts` переиспользован как есть):

```json
{
  "data": {
    "groups": [
      { "group": "STAGE_SHIFT", "totalReviewed": 5, "totalCorrect": 4, "accuracy": 0.78, "sampleEff": 4.6 },
      { "group": "DOCUMENTS", "totalReviewed": 3, "totalCorrect": 3, "accuracy": 0.92, "sampleEff": 2.9 },
      "...": "ещё четыре группы"
    ],
    "generatedAt": "…"
  }
...
`totalReviewed`/`totalCorrect` — «верно N из M последних» без забывания (всё время);
`accuracy` — доля с забыванием, `null` — наблюдений ещё не было; `sampleEff` —
эффективный объём наблюдений (чем он больше, тем увереннее число `accuracy`).

**Ошибки:** `VALIDATION_ERROR` 422 (тело, файл, `.eml` без адреса отправителя),
`NOT_FOUND` 404, `FORBIDDEN` 403, `UNAUTHORIZED` 401, `CONFLICT` 409 (уже проверено,
черновик недоступен до разбора), `INTERNAL` 500. `INTEGRATION_ERROR` не отдаётся: сбой
модели — запасной путь правилами или шаблон, как у ИИ-помощника.

**Журнал:** `inbound_letter.upload`, `inbound_letter.analyze` (payload: analyzedBy, group,
universityFound, fallbackReason), `inbound_letter.review` (verdict, group, universityId),
`inbound_letter.dismiss`, `inbound_letter.reply_draft`, `inbound_letter.reply_draft.edit` —
без текста письма и без содержимого черновика.

**Реестр DSAR:** `inbound_letters.reviewedById → User`,
`inbound_letter_tasks.responsibleId → User` (кто проверил письмо и кто отвечает за
задание — рабочие записи оператора, `erase: keep`, тем же принципом, что у связок
и рекомендаций). Само письмо (адрес и текст отправителя) — не сведения о сотруднике
ИТ-Школы, поэтому не выгружается и не обезличивается по запросу сотрудника; как
персональные данные вуза-корреспондента и на какой срок — `docs/PRIVACY.md`.
