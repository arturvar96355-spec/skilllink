...
**Это лента, а не постраничный список.** События собираются из нескольких источников —
создание связок, изменения этапов, документы, встречи — и сортируются по времени.
Поэтому здесь `limit`, а не `page`/`pageSize`, и в ответе **нет `meta`**: страницы
по такой склейке считать нечестно.
