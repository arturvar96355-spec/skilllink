...
> **22.09.2026.** Интерфейс собран по вашей дизайн-системе — решение 41
> в [TECHNICAL_DECISIONS.md](TECHNICAL_DECISIONS.md), устройство — в
> [FRONTEND.md](FRONTEND.md). Это задание осталось полезным как справка: запуск,
> вход, работа с API и типами описаны здесь и не изменились. Что делать дальше —
> смотреть готовые экраны и править то, что расходится с вашим замыслом:
> дизайн-система ваша, и последнее слово по внешнему виду за вами.

Серверная часть готова: 57 маршрутов, 71 операция, все проверены. Ниже всё, что нужно,
...
Репозиторий: https://github.com/arturvar96355-spec/skilllink — приватный. Доступ выдаёт Артур: пришлите ему свой ник
на GitHub, на почту придёт приглашение — его нужно принять, иначе репозиторий
не будет виден вовсе (GitHub отвечает «не найдено», а не «нет доступа»).
...
**Если вы на Windows — три отличия, каждое даёт непонятную ошибку, если о нём не знать:**

1. **Переменная перед командой не работает.** В PowerShell задавайте отдельно:
   `$env:POSTGRES_PORT = "5433"`, затем команда. В `cmd` — `set POSTGRES_PORT=5433`.
2. **`curl` в PowerShell — это псевдоним `Invoke-WebRequest`** с другим синтаксисом.
   Примеры из контракта API выдадут ошибку про несуществующий параметр. Пишите
   `curl.exe` — настоящий curl в Windows 10 и 11 есть.
3. **`openssl` обычно не установлен.** Где он упомянут, есть замена на Node.

Всё остальное — установка, запуск, миграции, демо-данные — работает одинаково.
Концы строк нормализуются автоматически, права на файлы не нужны, длинных путей
в проекте нет.
...
Так было на старте, 21.09. С 22.09 интерфейс собран, и структура — такая
(подробно — [FRONTEND.md](FRONTEND.md)):

```
src/
  app/
    (auth)/login/         вход
    (app)/                все внутренние страницы в общем каркасе
    layout.tsx            корень; providers.tsx подключает SessionProvider
    api/**/route.ts       НЕ ТРОГАТЬ: это серверная часть
  ui/                     дизайн-система: кнопки, таблицы, окна, списки выбора
  shared/contracts/       ← типы и подписи, импортируйте отсюда
  modules/                бизнес-логика, вам не нужна
```

Новые страницы — в `src/app/(app)/`, общие компоненты — в `src/ui/`.
