...
### Рейтинг вуза включается параметром

`GET /api/universities` по умолчанию отдаёт `rating: null` — это «не запрашивали».
Нужна колонка рейтинга, фильтр или сортировка по нему — добавьте `withRating=true`,
`minRating` / `maxRating`, `sort=-rating`. Отдельный параметр потому, что рейтинг
считается по всем программам сразу и реестру нужен не всегда.

В карточке вуза рейтинг приходит всегда, с полем `topProgram` — сильнейшей программой,
которой балл раскрывается.

Представителю вуза рейтинг недоступен: `403` на любой из этих параметров, `null` в карточке.
