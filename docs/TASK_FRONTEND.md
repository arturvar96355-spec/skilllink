...
Ване и Серёже. Серверная часть готова: 53 маршрута, 65 операций, все проверены. Ниже всё, что нужно,
...
**Проверьте версию Node перед установкой:** `node -v`. Нужна `^20.19` или `^22.12`
или `24+`. Диапазон странный, но это не придирка: Prisma 7 не работает на 20.0–20.18,
...
Опечатка в адресе тоже возвращает JSON — `404` с `code: "NOT_FOUND"`, а не
HTML-страницу. Если увидели `Unexpected token '<'` при разборе ответа, дело
не в адресе API.
...
### Рейтинг вуза приходит сам

`GET /api/universities` отдаёт `rating` в каждой строке без параметров. Фильтр
и сортировка — `minRating` / `maxRating`, `sort=-rating`.

`withRating=false` отключает расчёт: пригодится там, где реестр открывают ради выбора
из выпадающего списка, а не ради колонки рейтинга.

В карточке вуза есть `rating.topProgram` — сильнейшая программа, которой балл
раскрывается по клику.

Представителю вуза рейтинг недоступен: `403` на явный запрос, `null` в обычном списке
и в карточке. Отдельную колонку для этой роли просто не рисуйте — смотрите
`permissions.canSeeAnalytics` из `/api/me`.
...
| Срок вот-вот выйдет | `isDueSoon: true` | предупреждение, **не** тем же цветом, что просрочка — тут ещё есть время |
| Двойной клик по кнопке | 409 «изменено другим пользователем» | «обновите страницу» |
| Начать или завершить этап 6, 7 или 11 раньше времени | 409 + `details.blockingStages` | список этапов, которые надо закрыть — ссылками, а не текстом |
...
- **Лента событий вуза** — `/api/universities/:id/events`. Это лента, а не страницы:
  параметр `limit`, а в `meta` — `hasMore`. «Показать ещё» = запросить больший `limit`.
