...
| `npm run lint` | ESLint (`eslint.config.mjs`): ошибка — только настоящие, стиль — предупреждения |
...
| `npm run recs:simulate` | 90 дней решений менеджеров на объектах базы — как меняются веса правил рекомендаций (решение 119). База только читается; `--synthetic` — без базы, `--json файл` — ряды для графика |
| `npm run analytics:report` | Таблица этапов (медиана, p90, интервалы, порог застоя), воронка по вехам, когорты и «Система заметила» по базе из `DATABASE_URL`. Только чтение |
...
Две проверки (решение 118): `/api/health` — процесс жив и настроен (в базу не ходит),
`/api/ready` — база отвечает и миграции совпадают с кодом. Причину называет `/api/ready`:

| Ответ `/api/ready` | Что случилось | Что делать |
| --- | --- | --- |
| `status: ok` | всё работает | — |
...
| `degraded`, `reason: migrations-missing` или `migrations-pending` | миграции не применены | `npm run db:deploy` |
...
- **Проверка готовности видит неприменённые миграции:** `/api/ready` отдаёт 503 и `schema:
  "missing"`, пока база пуста, и `migrations-pending`, пока в коде есть миграции новее
  применённой. Healthcheck контейнера смотрит на `/api/health` — живость без базы: падение
  базы не делает контейнер «нездоровым» (решение 118).
...
### Каналы уведомлений: MAX и VK

Тот же блок «Каналы уведомлений» в личном кабинете (решение 144) умеет ещё два канала —
MAX (мессенджер VK, боты через Bot API) и VK (бот сообщества, `messages.send`). На этом
хакатоне настоящих токенов MAX и VK нет и не будет: оба канала намеренно оставлены
«для галочки» — видны в интерфейсе, честно пишут «Не настроено администратором», ничего
не падает и не требует ключей, чтобы приложение работало. Ниже — как подключить
по-настоящему, когда это понадобится; без этого раздела можно просто продолжать
пользоваться Telegram.

#### MAX

**1. Создать бота.** В приложении MAX (или через бота «MAX для бизнеса») — раздел ботов,
«Создать бота»: имя, оканчивающееся, например, на `_bot`, и команда «Получить токен».
Токен — секрет, в git и в чаты его не вставлять.
...
# Секрет вебхука — придумывается заранее, MAX его не выдаёт сам:
# node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"
MAX_WEBHOOK_SECRET=<секрет>
```

Перезапустить приложение (docs/DEPLOY.md, «Как подключить YandexGPT», шаг 2 — тот же
порядок для любой новой переменной).

**3. Подписать вебхук.** MAX должен знать адрес и секрет — `POST /subscriptions` Bot API
MAX с `url: "https://<домен>/api/channels/max/webhook"`, `secret: <тот же MAX_WEBHOOK_SECRET>`,
`update_types: ["message_created","bot_started"]` (см. документацию dev.max.ru/docs-api —
на момент написания раздел методов подписки; точные поля стоит сверить с актуальной
версией перед боевым подключением). Без верного заголовка `X-Max-Bot-Api-Secret` вебхук
отвечает 403 — так и задумано.

**4. Проверить.** Войти сотрудником с правом на сводку → «Личный кабинет» → «Каналы
уведомлений» → MAX → «Подключить» → открыть диплинк и написать боту: ответит
«Готово: подключено», строка в кабинете покажет «Подключено». «сегодня» — сводка,
«стоп» — отключить.

#### VK

**1. Создать сообщество и бота.** Создать сообщество ВКонтакте (или использовать
существующее) → «Управление» → «Работа с API» → создать токен сообщества с доступом
«Сообщения сообщества». Отдельно — «Управление» → «Сообщения» → включить возможность
писать сообществу.

**2. Включить Callback API.** «Управление» → «Работа с API» → «Callback API»: указать
адрес `https://<домен>/api/channels/vk/callback`, задать секретный ключ (поле «Секретный
ключ») — это и есть `VK_SECRET`. VK покажет код подтверждения адреса — это `VK_CONFIRMATION_CODE`.
Включить события «Входящее сообщение» (`message_new`).

**3. Переменные.** На сервере — в `~/skilllink/.env.cloud` (права 600), локально — в `.env`:

```bash
VK_GROUP_TOKEN=<токен сообщества с правом «Сообщения сообщества»>
VK_GROUP_ID=<id сообщества, число без минуса>
VK_CONFIRMATION_CODE=<код из настроек Callback API>
VK_SECRET=<секретный ключ из тех же настроек>
```

Перезапустить приложение, затем в настройках Callback API нажать «Подтвердить адрес» —
VK обратится на `/api/channels/vk/callback` с `type: "confirmation"`, маршрут ответит
кодом открытым текстом.

**4. Проверить.** «Личный кабинет» → «Каналы уведомлений» → VK → «Подключить» → перейти
по ссылке `vk.me/club<id>?ref=<код>` и написать что угодно первым сообщением: VK вернёт
метку `ref` в событии `message_new`, бот ответит «Готово: подключено».

**Проверка обоих каналов администратором** — «Настройки → Интеграции» → «Проверить»
(`POST /api/admin/channels/{max|vk}/test`): пробное сообщение в собственный чат
администратора — сначала он должен подключить канал себе.

**Long polling вместо вебхука.** Если вебхук недоступен (нет публичного HTTPS-адреса на
время разработки), у обеих платформ есть режим опроса без вебхука — `GET /updates` у MAX
и Long Poll API у VK (`groups.getLongPollServer`). В этой версии он не реализован —
маршруты `/api/channels/{max,vk}/*` рассчитаны на вебхук; добавить опрос можно отдельным
скриптом по образцу `scripts/telegram-digest.ts`, вызывающим `notify-channels.service`
теми же функциями (`acceptUpdate`, `handleInbound`), которыми пользуются вебхуки.
...
**5. Приём без вебхука — long polling (решение 142).** На стенде в Yandex Cloud исходящие
к api.telegram.org идут через `TELEGRAM_API_IP` и работают, но входящие от Telegram к
вебхуку иногда не достукиваются (`getWebhookInfo` → `last_error_message: Connection timed
out`) — сеть облака отдаёт исходящие соединения охотнее входящих. Переменная `TELEGRAM_MODE`
решает, как приложение принимает обновления:

- `webhook` — только вебхук, как раньше;
- `polling` — процесс сам держит цикл `getUpdates` (снимает вебхук при старте), вебхук не нужен;
- `auto` (по умолчанию, включая пустое значение) — вебхук, пока отвечает; если последняя
  ошибка вебхука свежее 10 минут или необработанных обновлений становится больше, процесс
  сам переключается на `polling` (проверка раз в 5 минут), с записью в журнал действий и
  оповещением владельцу в Telegram.

Режим читается при старте приложения и меняется на лету через админку — «Настройки →
Интеграции» → «Бот Telegram» (только `ADMIN`, `GET`/`PUT /api/admin/telegram/...`, право
`ADMIN`): статус, смена и удаление токена (проверяется у Telegram до сохранения, хранится
в базе зашифрованным — главнее переменных окружения), переключатель режима, «Сменить
секрет вебхука», «Отправить проверочное сообщение». Смена токена или режима в админке
применяется сразу, без перезапуска процесса.

**На этом стенде, пока вебхуку не достучаться, рабочий выбор — `polling` или `auto`**
(поставить в `.env.cloud` `TELEGRAM_MODE="polling"` или оставить `auto` и дать процессу
переключиться самому при следующей проблеме с вебхуком). Ограничение: цикл polling и
auto-проверка живут в памяти одного процесса приложения — при нескольких экземплярах
(несколько реплик за балансировщиком) нужен либо один выделенный процесс-приёмник, либо
возврат к вебхуку (docs/SECURITY_LIMITATIONS.md).
