...
остаётся тихой опечаткой. Остальные строки — из прогона на 26.09.2026 (решение 141);
при последующих изменениях сверяются заново тем же способом (`npm test`, `npm run
db:verify`, `npm run smoke`, `npm run probe`, `npm run demo:check`, `npm run demo:coverage`).
