...
205 тестов: таблица переходов этапов, условия завершения и блокировки, автоматический пересчёт
...
205 проверок по разделам 17 и 18 ТЗ: подключение к базе, дашборд, реестр вузов, создание вуза
...
встречи, кабинет представителя вуза и интеграции.
...
14. Открыть кабинет представителя вуза — `GET /api/portal/overview` с cookie представителя.

### Выбор пользователя в демо

На P0 работает mock-авторизация. Текущий пользователь берётся из cookie `skilllink_user`
со значением идентификатора. Идентификаторы демо-пользователей выводит `npm run db:seed`.
...
Без cookie берётся первый активный сотрудник.

Текущая роль и её права — `GET /api/auth/me`. Справочник пользователей — `GET /api/users`.

Среди демо-пользователей есть представитель вуза (`UNIVERSITY_REP`): под его cookie видно,
что кабинет показывает только свой вуз, а аналитика и рекомендации закрыты.
