...
## Для эксперта за 5 минут

- Стенд — **https://skilllink.site**; логины — раздел [«Попробовать»](#попробовать) ниже,
  пароль — по каналу организаторов конкурса.
- **Пароль вводить не нужно** — на странице входа под формой кнопки «Менеджер»,
  «Администратор», «Представитель вуза» (решение 176): один клик входит в готовую
  учётную запись эксперта. Кнопки видны, только когда организаторы включили их для
  показа; если их нет — вход обычным способом, как в предыдущем пункте.
- Главный путь в 5 кликов — первые пять шагов раздела
  [«Проверить за 10 минут»](#проверить-за-10-минут).
- Соответствие ТЗ построчно (пункт → экран → API → решение) —
  [docs/TZ_COMPLIANCE.md](docs/TZ_COMPLIANCE.md).
- Все эндпоинты и схемы — Swagger UI на стенде:
  [`skilllink.site/api-docs`](https://skilllink.site/api-docs).
...
| `HEAD` | `head@skilllink.demo` | руководитель направления сотрудничества с вузами | всё, что менеджер, плюс назначение и смена ответственного по вузу и связке (решение 146) |
...
| `VIEWER` | `viewer@skilllink.demo` | наблюдатель | аналитика и реестры, без правки |
...
**Отдельные учётные записи для экспертов конкурса** (решение 147) — тот же пароль, но
разрушающие и изменяющие действия сервер отклоняет независимо от роли (403 «Недоступно
в учётной записи эксперта: действие необратимо»): удаление, архивирование, обезличивание,
слияние, блокировка и смена роли пользователей, загрузка и удаление файлов, импорт и
похожее. Экспертам открыты чтение и выгрузки — этого достаточно, чтобы посмотреть систему
и ничего в ней не сломать.

| Роль | Логин | Что видит |
| --- | --- | --- |
| `MANAGER` | `expert-manager@skilllink.demo` | всё, что менеджер, без права менять |
| `ADMIN` | `expert-admin@skilllink.demo` | всё, что администратор видит, без права менять |
| `UNIVERSITY_REP` | `expert-rep@skilllink.demo` | кабинет вуза СПбГУТ, без права подтверждать за вуз |

**Вход в эти три учётные записи — в один клик** (решение 176): на странице входа под
формой — кнопки с их названиями, пароль вводить не нужно. Кнопки видны, только когда на
стенде включена переменная `EXPERT_QUICK_LOGIN`, и сервер на каждый клик заново проверяет
по базе, что это действительно одна из учётных записей эксперта — обычным входом это не
воспользоваться, даже зная адрес почты.
...
Роли и логины — по фактической истории коммитов (`git shortlog -sne`), не выдуманные:

- **Артур** (GitHub [arturvar96355-spec](https://github.com/arturvar96355-spec), владелец
  репозитория) — постановка задач и приёмка результатов, документация и сопроводительные
  материалы к сдаче, архитектурные развилки. Работа в основном вне коммитов в `main`
  (задачи, ревью, документы), поэтому в `git shortlog` не виден.
- **Incha1ns** — сервер, база и эксплуатация: API, схема и миграции, машина состояний
  этапов, контейнеризация и развёртывание; постановка задач ИИ-агентам и разбор
  результата за них.
- **Сергей Боровицкий** (GitHub `pels18`) — интерфейс и дизайн-система: реестры, карточка
  связки, кабинет представителя вуза, дашборд.
- **Tigraaan** — проверки базы данных: целостность схемы, миграции, контрольные запросы.
- **Ваня** — помощь на фронтенде; без коммитов в `main`.

Решения PM по продукту и процессу, с датой и причиной, — [docs/DECISIONS.md](docs/DECISIONS.md);
технические решения (архитектура, альтернативы, что проверено) — весь журнал в
[docs/TECHNICAL_DECISIONS.md](docs/TECHNICAL_DECISIONS.md) (не фиксируем номер последней
записи здесь, чтобы строка не расходилась с журналом при каждом новом решении).

**Честно про ИИ.** Основной объём кода написан ИИ-ассистентом (Claude Code) по заданиям
команды — задачу, контекст и критерии приёмки формулировал человек, результат проверяли
CI и человек перед вливанием в `main`. Контроль: CI на каждый PR (типы, тесты, сборка,
сверка OpenAPI и документов с кодом), сверка живого стенда со сценарием показа перед
выкладкой, журнал решений на каждую спорную развилку. Обязательное ревью каждого PR
человеком на время хакатона заменено обязательными проверками CI — сроки не позволяли
держать оба; после сдачи обязательное ревью возвращается.
...
| Что | Число |
| --- | --- |
| Моделей в схеме (`prisma/schema.prisma`) | 50 |
| Миграций (`prisma/migrations`) | 32 |
| Маршрутов API (`src/app/api/**/route.ts`) | 152 |
| Операций в спецификации (`docs/openapi.json`) | 183 |
| Тестов (`npm test`) | 3487 (3469 выполняются, 18 пропущено — 16 требуют TEST_DATABASE_URL, 2 — маршруты без отдельного описания) |
| Правил целостности данных (`npm run db:verify`) | 36 (46 с демо-набором) |
| Проверок сквозного сценария (`npm run smoke`) | 350 |
| Проверок пробника (`npm run probe`) | 587 |
| Пунктов сверки со сценарием показа (`npm run demo:check`) | 28 |
| Полнота демо-данных (`npm run demo:coverage`) | 50 модели, 0 пустых содержательных таблиц |
...
остаётся тихой опечаткой. Число тестов сверено `npx vitest run` при решении 173
(26.09.2026); строки про `db:verify`, `smoke`, `probe`, `demo:check` и
`demo:coverage` — из прогона на 26.09.2026 (решение 141), а не пересчитаны заново
здесь: это прогоны против поднятой базы и dev-сервера, вне точечной правки.
При последующих изменениях все строки сверяются заново тем же способом (`npm test`,
`npm run db:verify`, `npm run smoke`, `npm run probe`, `npm run demo:check`,
