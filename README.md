...
Правила и приёмы — [docs/FRONTEND.md](docs/FRONTEND.md). Сам стандарт оформления —
шесть документов в [docs/design/](docs/design/). Значения из них заданы переменными
...
суток числа на экране расходятся со сценарием ([docs/DEPLOY.md](docs/DEPLOY.md)).
...
| [docs/DEMO_CARD.md](docs/DEMO_CARD.md) | Шпаргалка докладчику: куда нажать, что появится, что сказать |
...
| [docs/FRONTEND.md](docs/FRONTEND.md) | **Интерфейс:** дизайн-система, правила, загрузка данных |
| [docs/design/](docs/design/) | Дизайн-система: шесть документов от дизайнеров |
| [docs/DESIGN_INTEGRATION.md](docs/DESIGN_INTEGRATION.md) | Связь экранов с API: какое поле питает какой блок |
...
| `AI_ASSIST_PROVIDER` | ИИ-помощник: `off` (по умолчанию), `yandexgpt`, `gigachat` — решение 90, подключение в [docs/DEPLOY.md](docs/DEPLOY.md) |
...
- **Вход — по паролю** (NextAuth, bcrypt, пять неудачных попыток закрывают вход
  на 15 минут). Демо-режим без пароля на стенде выключен — это проверяет
  `scripts/deploy/check.sh`. Политик на уровне строк (RLS) в базе нет, доступ
  разграничен в сервисах — [SECURITY_LIMITATIONS.md](docs/SECURITY_LIMITATIONS.md).
...
- Сергей — серверная часть, API, workflow, аналитика; сборка интерфейса по дизайн-системе
