 ...
 *
 * Content-Security-Policy — только директивы, которые не касаются скриптов, стилей,
 * шрифтов и картинок: Next вставляет встроенные скрипты, и строгая политика без
 * проброса nonce сломает приложение молча — на проде в самый неудачный момент.
 * Полная политика — отдельная задача, описана в SECURITY_LIMITATIONS.md.
 */
const SECURITY_HEADERS = [
  ...
  // Часть CSP, безопасная без nonce: не встраиваться в чужие страницы (то же, что
