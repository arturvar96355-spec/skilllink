...
#
# Иначе после выкладки браузер показывает прошлую версию стенда, и пользователь
...
# обязан эту страницу открыть, иначе вход и главная перенаправляют друг
# на друга, и экран остаётся пустым (решение 48).
