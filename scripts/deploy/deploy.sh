...
# сети, VPN и мобильные операторы обрывают такие сами (чаще всего — на `npm ci`).
#
...
# которая обновляет стенд по этой отметке, не должна считать его свежим.
...
# несколько секунд. Проверка, запущенная сразу, показала бы провалы при
# исправном стенде — поэтому сначала ждём сертификат.
if [ -n "$DOMAIN" ]; then
