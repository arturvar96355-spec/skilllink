...
async function main(): Promise<void> {
  ...
  check(
    ...
    'Просрочен этап 5: Доработка документов при необходимости',
  )
  ...
  check(
    'сверху три критичные просрочки',
  ...
  check(
    'под ними дефициты навыков',
    recommendations.slice(3, 5).map((item) => item.title),
    [
